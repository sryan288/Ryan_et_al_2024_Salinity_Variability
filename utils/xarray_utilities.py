...
'''
    ...
    5) cut2fullyears                 Removes years that are not complete at beginning or end
...
#
#----------------------------------------------------------------------------------
# 7) remove data that does not have full year and beginnging or end (daily))
def cut2fullyears(ds,timevar='time'):
    # check time step first [days]
    dt = int(ds[timevar].diff(dim=timevar)[10].values/(24 * 60 * 60 * 1e9))
    if dt==1: yearlen = 365
    elif dt>25: yearlen = 12
    # find first and list year
    years = ds[timevar+'.year']
    year1 = np.min(years)
    yearend = np.max(years)
    
    # check if full year
    if len(ds[timevar].where(years==year1,drop=True))<yearlen:
        year1 = year1+1
    if len(ds[timevar].where(years==yearend,drop=True))<yearlen:
        yearend=yearend-1
